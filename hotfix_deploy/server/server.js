...
app.post('/api/teams', upload.fields([{ name: 'logo', maxCount: 1 }, { name: 'captain_photo', maxCount: 1 }]), async (req, res) => {
  const logo = req.files?.logo?.[0] ? await mediaStorage.saveMulterFile(req.files.logo[0], 'uploads', req.files.logo[0].originalname || 'team_logo.png') : null;
  const captain_photo = req.files?.captain_photo?.[0] ? await mediaStorage.saveMulterFile(req.files.captain_photo[0], 'uploads', req.files.captain_photo[0].originalname || 'captain_photo.png') : null;
...
app.put('/api/teams/:id', upload.fields([{ name: 'logo', maxCount: 1 }, { name: 'captain_photo', maxCount: 1 }]), async (req, res) => {
  const team = db.getTeam(parseInt(req.params.id));
  if (!team) return res.status(404).json({ error: 'Not found' });
  const logo = req.files?.logo?.[0] ? await mediaStorage.saveMulterFile(req.files.logo[0], 'uploads', req.files.logo[0].originalname || 'team_logo.png') : team.logo;
  const captain_photo = req.files?.captain_photo?.[0] ? await mediaStorage.saveMulterFile(req.files.captain_photo[0], 'uploads', req.files.captain_photo[0].originalname || 'captain_photo.png') : team.captain_photo;
